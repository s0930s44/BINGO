...
// 初始化 SQLite 數據庫
const db = new Database(path.join(DATA_DIR, 'bingo-game.db'));
...
function initializeDatabase() {
    try {
        ...
        // 建立索引以提升查詢效能
        db.exec(`CREATE INDEX IF NOT EXISTS idx_users_room ON users(room)`);
        db.exec(`CREATE INDEX IF NOT EXISTS idx_drawn_numbers_room ON drawn_numbers(room)`);
...
io.on('connection', (socket) => {
    ...
    socket.on('login', (data) => {
        try {
            ...
            if (!isAdmin) {
                // 檢查房間是否存在
                const roomExists = db.prepare(`SELECT * FROM rooms WHERE room_name = ?`)
                    .get(data.room);
                ...
                if (!roomExists) {
                    db.prepare(`INSERT INTO rooms (room_name, user_count) VALUES (?, 0)`)
                        .run(data.room);
                }
            }
            ...
            db.prepare(`
                ...
                VALUES (?, ?, ?, ?, 0)
            `).run(socket.id, data.username, data.room, isAdmin ? 1 : 0);
            ...
            if (roomTimers[data.room]) {
                ...
                db.prepare(`DELETE FROM room_timers WHERE room = ?`)
                    .run(data.room);
            }
    ...
    try {
        ...
        for (const room of rooms) {
            ...
            db.prepare(`
                ...
                WHERE room_name = ?
            `).run(userCount.count, room.room_name);
        }
